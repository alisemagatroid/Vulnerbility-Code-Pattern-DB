...
def match_units(query_units, sig_units, tag_to_idx, is_block=False, top_k=2):
    ...
    for q in query_units:   # 쿼리 구간(statement/block)별로 루프
        ...
        for s in sig_units:  # 시그니처의 모든 구간과 비교
            ...
            if sim > best_sim:
                best_sim, best_sig, best_tags, best_embedding_sim = sim, s, s.get("tags", []), emb_sim
...
def match_critical_evidence(query_variant, db_variant, level="tag"):
    ...
    # 1. 태그 집합 준비
    if level == "tag":
        query_tags_set = [set(s.get("tags", [])) for s in query_variant.get("statement_slices", [])]

    # 2. 코드 조각 집합 준비
    elif level == "code":
        query_slices = [s.get("code") for s in query_variant.get("statement_slices", [])]
    else:
...
def tag_diff_summary(query_tags, sig_tags):
    ...
    for tag in all_tags:
        if tag in query_tags and tag in sig_tags:
            summary.append({"tag": tag, "status": "match", "explanation": "둘 다 존재"})
        elif tag in query_tags:
            summary.append({"tag": tag, "status": "query_only", "explanation": "Query에만 존재"})
        else:
            summary.append({"tag": tag, "status": "signature_only", "explanation": "시그니처에만 존재"})
    return summary
...
def block_structure_rule_match(blocks, rule_entry, level=0, parent_idxs=None, debugMode=False):
    ...
    if debugMode: print(f"{prefix}[DEBUG] Rule: {rule_entry} / Block Types: {block_infos}")
    ...
    if isinstance(rule_entry, str):
       # 문자열 규칙: 태그인지 구조블록인지 구분
        if rule_entry.startswith('[') and rule_entry.endswith(']'):
                if debugMode: print(f"{prefix}[DEBUG] TAG Entry: {rule_entry['or']}")
                tag = rule_entry
                found_idxs = [
                    b["idx"] for b in blocks
                    if tag in b.get("tags", []) and
                    (parent_idxs is None or any(str(b["idx"]).startswith(f"{pid}.") for pid in parent_idxs))
                ]
                return bool(found_idxs), found_idxs
        
        if parent_idxs is None:
            found_idxs = [b["idx"] for b in blocks if b["block_type"] == rule_entry]
        ...
        if debugMode: print(f"{prefix}[DEBUG] '{rule_entry}' FOUND: {found_idxs}")
        return found, found_idxs
    

    # 2. OR 연산
    elif isinstance(rule_entry, dict) and "or" in rule_entry:
        if debugMode: print(f"{prefix}[DEBUG] OR block: {rule_entry['or']}")
        for sub in rule_entry["or"]:
            ok, idxs = block_structure_rule_match(blocks, sub, level+1, parent_idxs=parent_idxs)
            if ok:
                if debugMode: print(f"{prefix}[DEBUG] OR result: True (matched: {sub} with idxs: {idxs})")
                return True, idxs
        if debugMode: print(f"{prefix}[DEBUG] OR result: False")
        return False, []
    ...
    # 3. IN (계층적) 연산: e.g., {in: [A, B, C]} → A→B→C로 내려가는 체인
        ...
        if parent_idxs is None:
            ...
            all_parent_idxs = [b["idx"] for b in blocks if b["block_type"] == chain[0]]
            for pid in all_parent_idxs:
                ok, idxs = block_structure_rule_match(blocks, {"in": chain}, level+1, parent_idxs=[pid], debugMode=debugMode)
                if ok:
                    return True, idxs
            return False, []
        else:
            if not parent_idxs:
                return False, []
            if len(chain) == 1:
                if debugMode: print(f"{prefix}[DEBUG] End of chain, found parent_idxs={parent_idxs}")
                    ...
                    if str(b["idx"]).startswith(pstr):
                        ...
                        if (isinstance(next_type, str) and next_type.startswith("[") and next_type.endswith("]")):
                            if next_type in b.get("tags", []):
                                if debugMode:
                                    print(f"{prefix}[DEBUG] TAG MATCH: {b['idx']} has {next_type}")
    ...
    # 4. AND 연산: 모든 조건 만족해야 함
        ...
        if debugMode: print(f"{prefix}[DEBUG] AND block: {rule_entry}")
        ok, parent_idxs = block_structure_rule_match(blocks, rule_entry[0], level+1, parent_idxs=parent_idxs, debugMode=debugMode)
        if not ok:
            if debugMode: print(f"{prefix}[DEBUG] AND result: False (first condition)")
            return False, []
        for sub in rule_entry[1:]:
            ok, parent_idxs = block_structure_rule_match(blocks, sub, level+1, parent_idxs=parent_idxs, debugMode=debugMode)
            if not ok:
                return False, []
        return True, parent_idxs
        
    if debugMode: print(f"{prefix}[DEBUG] Unmatched type for rule_entry: {rule_entry}")
...
def signature_pattern_match(query_variant, signature_pattern, debugMode=False):
    ...
    """
    if debugMode: print(f"DEBUG:[signature_pattern_match]::signature_pattern: {signature_pattern}")

    q_tags = set()
    for s in query_variant["statement_slices"]: q_tags.update(s.get("tags", []))
    ...
    if debugMode: print(f"DEBUG::sequence_matched_up_to = {seq_ptr}, seq_match = {seq_match}")
    if not seq_match:
    ...
    block_ok = True
    if block_struct:
        block_ok = block_structure_rule_match(q_blocks, block_struct,debugMode=False)
        if not block_ok:
            fail_reason.append(f"Block structure not matched. (required: {block_struct})")
...
def explainable_search_report(
    ...
    for db in variant_db:
       ...
       # (e) Hybrid: 임베딩 + one-hot TAG + Jaccard + TF-IDF
        hybrid_sim = (
        ...
        # print(f"[{count}]{db['pattern_id']}: emb={emb_sim:.3f} tag={tag_sim:.3f} "
        #       f"jac={jac_sim:.3f} tfidf={tfidf_sim:.3f} hybrid={hybrid_sim:.3f}")
    
    # ── 4)  Top-N 유사도가 높은 variant들 선택
    # hybrid_sim이 높은 순서대로 report를 생성
    scored_variants = sorted(scored_variants,
                             key=lambda x: -x["hybrid_sim"])[:top_k]
    # ── 5) Signature DB 로드
    sig_db = load_signature_patterns(signature_yaml)

    # ── 6) Top-N variant에 대해 상세 비교 
    results = []
    for sv in scored_variants:
        ...
        # critical_evidence의 매치와 페널티 점수를 조합해서, 점수를 매기나, matches가 비어있다
        # 현재 태그가 완전히 매치되지 않으면 matches는 구해지지 않는다.
        # 지금 태그의 비교는 variant_db.json의 critical_evidence를 기반으로 매치 여부를 확인한다.
        matches = match_critical_evidence(query_variant, db_var, level=evidence_level)# qv: 쿼리 함수 db_var: topk내의 함수들 전부       
        ...
        if pid and pid in sig_db:
            ...
            # topk내의 모든 코드 패턴에 대한 yaml파일 내의 정보
            sig_ref = {
                "pattern_id": pid,
...
if __name__ == "__main__":
    ...
    # report를 return하는 함수
    report_json  = explainable_search_report(query_variant, db_variants, SIG_DB_YAML, top_k=1)
